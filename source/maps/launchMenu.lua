function launchMenu()
    ...
    table.insert(tutorial_elements, Text(params.winWidth/2, 380, "Press ESC to pause.", 1, txtCol, buttonCol))
    table.insert(tutorial_elements, Text(params.winWidth/2, 420, "Score as many points as you can before the timer runs out!", 1, txtCol, buttonCol))
    table.insert(tutorial_elements, Text(params.winWidth/2, 500, "Bird Behaviour Handbook, pp 26: 'Birds typically prefer to perch on certain tree species. Watch carefully how they move.", 1, txtCol, buttonCol))
    table.insert(tutorial_elements, Text(params.winWidth/2, 540, "However be cautios, when approached by humans they can become frightened (!)'.", 1, txtCol, buttonCol))
